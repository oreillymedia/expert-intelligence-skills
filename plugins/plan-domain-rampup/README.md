...
A 30-60-90 plan with observable milestones, the reading and events that support each phase, and
agendas for the conversations to have with the people who already own the domain. Sequenced so
you're credible early rather than well-read late.
...
4. Install **Ramp Plan** from the list.
...
/plugin install plan-domain-rampup@expert-intelligence-skills
```
...
codex plugin add plan-domain-rampup@expert-intelligence-skills
```
...
Download
[SKILL.md](https://github.com/oreillymedia/expert-intelligence-skills/blob/main/plugins/plan-domain-rampup/skills/plan-domain-rampup/SKILL.md)
...
- **Claude Code or Codex CLI:** save it, filename unchanged, as `plan-domain-rampup/SKILL.md` inside
...
`description` field in `skills/plan-domain-rampup/SKILL.md` for the full set of trigger conditions.
